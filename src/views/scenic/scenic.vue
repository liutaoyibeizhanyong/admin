<template>
  <div class="user">
    <el-card class="box-card">
      <table-data :autoHeight = "autoHeight" :tableList = "tableList" :tableHead = "$tableHead.scenic.tableHead" ></table-data>
...
import {post_scenic } from "@/assets/js/request/main";
export default {
  data() {
    return {
      count:0,//总页数
  ...
  methods: {
    ...
    async getHotelCheckIn() {
      ...
      let { list, code ,count} = await post_scenic(that.params);
      if (code === 200) {
        //表格数据tableList
        that.tableList = list;
        //表格总页数count
